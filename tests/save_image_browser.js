...
try {
    for (const accessor of [false, true]) {
        ...
        let wheels=0, spaces=0, panStarts=0;
        const wheelListener=()=>wheels++;
        const keyListener=e=>{if(e.code==='Space')spaces++;};
        const pointerListener=()=>panStarts++;
        card.addEventListener('wheel',wheelListener);
        card.addEventListener('keydown',keyListener);
        card.addEventListener('pointerdown',pointerListener);
        const wheel=new WheelEvent('wheel',{bubbles:true,cancelable:true,deltaY:100});stage.dispatchEvent(wheel);
        check(wheels===1 && !wheel.defaultPrevented,'Wheel reaches canvas without cancellation');
        const beforePan=stage.getAttribute('aria-valuenow');
        stage.dispatchEvent(new KeyboardEvent('keydown',{key:' ',code:'Space',bubbles:true}));
        const pan=new PointerEvent('pointerdown',{button:0,pointerId:91,bubbles:true,cancelable:true});stage.dispatchEvent(pan);
        stage.dispatchEvent(new KeyboardEvent('keyup',{key:' ',code:'Space',bubbles:true}));
        check(spaces===1 && panStarts===1 && !pan.defaultPrevented && stage.getAttribute('aria-valuenow')===beforePan,'Space-drag reaches canvas without moving divider');
        stage.dispatchEvent(new MouseEvent('click',{bubbles:true}));
        const middle=new PointerEvent('pointerdown',{button:1,pointerId:92,bubbles:true,cancelable:true});stage.dispatchEvent(middle);
        check(panStarts===2 && !middle.defaultPrevented,'Middle-button pan reaches canvas');
        stage.dispatchEvent(new MouseEvent('click',{bubbles:true}));
        card.removeEventListener('wheel',wheelListener);card.removeEventListener('keydown',keyListener);card.removeEventListener('pointerdown',pointerListener);

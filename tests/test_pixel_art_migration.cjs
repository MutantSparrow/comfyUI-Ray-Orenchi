...
assert.deepEqual(Array.from(node.widgets_values), ["repair_pixel_art", "auto_pixel_size", 96, 8, "grid_snap", true, 24,
                                                 "source", "error_diffusion", .7, "silhouette", "ramps_oklab", true, 90, false, 4, "area_preserving", 42, "fixed"]);
...
assert.deepEqual(Array.from(previous.widgets_values),["repair_pixel_art","pixel_size",64,6,"nearest",false,16,"source","none",.5,"none","kmeans_lab",true,90,false,4,"area_preserving",123,"fixed"]);
...
assert.deepEqual(Array.from(v2.widgets_values),["illustration_photo","manual_resize",128,8,"area",true,32,"distinct","ordered",.3,"none","kmeans_lab",true,90,false,4,"area_preserving",19,"randomize"]);
...
assert.deepEqual(Array.from(recent.widgets_values.slice(15)),[4,'frequency',42,'fixed']);
...
function Node() {this.widgets=[
    ...["pixel_size","target_resolution","max_colors","palette_style","palette_allocation","dither_strength","highlight_threshold","ramp_levels"].map(name=>({name})),

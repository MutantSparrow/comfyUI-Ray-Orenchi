...
| Ray's Local: Folder Image Scraper | Random image + extracted prompt from a local folder. Reads A1111 `parameters`, ComfyUI `prompt` / `workflow` chunks, JPEG/WEBP EXIF UserComment, and `<image>.txt` sidecars. Follows wired text inputs back through Show Text / Text Multiline / String Literal / concat nodes. Recurse-subfolders toggle, skip-if-no-prompt toggle, `image_path` output. Multi-prompt images emit one list entry per prompt across all outputs. |
